...
func RouterStart() {
	...
	g.DELETE("/xsskc", handler.Delxsskc)             // 删除限时赛指定的库存牌局
	...
	g.DELETE("/xssyx", handler.Delxssyx)             // 删除限时赛指定桌子上的指定牌局
	g.POST("/xssyx/order", handler.Changexssyxorder) // 改变限时赛指定牌局的order属性
	g.POST("/xssyx",handler.Diyxssyx)                // 自定义限时赛指定桌子的指定牌局
